        ...
        if(reservation.value!=null){
            ...
            db.document(reservation!!.value!!.id!!.path).update("participants" , arraytmp).addOnSuccessListener {
                db.document(reservation!!.value!!.id!!.path).update("confirmed_participants",confermati).addOnSuccessListener {
                    ...
                    Notification(
                        message = "${main.getStringFromId(context=cont,id= R.string.the_user)}" +user.username+ "${main.getStringFromId(context=cont,id= R.string.accepted_reservation)}" + reservation.value!!.date + " " + reservation.value!!.time + " ",
                        recipient = auth_user,
                        is_read = false,
